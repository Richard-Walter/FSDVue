...
$(document).ready(function(){
  ...
  // $(document).on("click", ".iw_update_poi", function () {
          
  //   var eventId = $(this).data('id');
  //   var form_action = '/poi/' + eventId + '/update'+'?page=home'
  ...
  // handle for importing flight plan from file
  $('#fp_import_btn').on('click', function() {
    $('#fp_file_import').trigger('click');
  });


  // handle for showing/hiding recorded flights
  $('#show_my_flights_check').on('click', function() {
    let show_flights_checked = $('#show_my_flights_check').is(":checked")

    if (show_flights_checked){
      $('#show_my_flights_check').val('Yes');

      if (rf_polylines) {
        for (var i = 0; i < rf_polylines.length; i++) {

          sim_flight_path = rf_polylines[i];
          sim_flight_path.setMap(map);
        }
      }
      if (rf_polygons) {
        for (var i = 0; i < rf_polygons.length; i++) {

          flight_polygon = rf_polygons[i];
          flight_polygon.setMap(map);
        }
      }


    } else {
      $('#show_my_flights_check').val('No');
      if (rf_polylines) {
        for (var i = 0; i < rf_polylines.length; i++) {

          sim_flight_path = rf_polylines[i];
          sim_flight_path.setMap(null);
        }
      }
      if (rf_polygons) {
        for (var i = 0; i < rf_polygons.length; i++) {
  ...
  $('#fp_file_import').on('change', function(e) {
    var file = e.target.files[0];
    try {
      importFlightPlanFromFile(file);
      ...
      //reset file dialog so on change gets trigger when user  imports same file twice
      document.getElementById("fp_file_import").value = "";
    }
    
  });

  //handle for saving a recorded flight from flight plan
  $(document).on("click", "#delete_flight_confirm_btn", function () {
        
      $("body").css("cursor", "progress");
  });

   //handle for saving a recorded flight from flight plan
   $(document).on("click", "#save_fp_as_recorded_flight", function () {
          
    save_fp_as_recorded_flight();
  });
  ...
  $("a.boxed-btn3").click(function(){
    ...
    let zoom_data = $(this).attr("data-value").split(",");
    let lat = parseFloat(zoom_data[0]);
    let lng = parseFloat(zoom_data[1]);
    let zoom_lvl = parseInt(zoom_data[2]);

    map.setZoom(zoom_lvl);      // This will trigger a zoom_changed on the map
  ...
  $('#highest_rated_airports').DataTable( {
  ...
  $('#account_visited_airport_datatable').DataTable( {
  ...
  $('#account_favorite_airport_datatable').DataTable( {
