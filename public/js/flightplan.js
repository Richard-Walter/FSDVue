...
// var flightplan_saves_as_recorded_flight = false;
...
// function importFlightPlanFromFile(file) {
...
//         flightPath_data = [];
//         var x_title = xmlDoc.getElementsByTagName("Title");
//         var fp_title = x_title[0].childNodes[0].nodeValue;

//         var x_cruise_alt= xmlDoc.getElementsByTagName("CruisingAlt");
//         var fp_cruise_alt = x_cruise_alt[0].childNodes[0].nodeValue;

//         var x_waypoints= xmlDoc.getElementsByTagName("ATCWaypoint");
      
//         console.log(x_waypoints);

//         for (var elem of x_waypoints) {
//           console.log(elem)
//           var fp_name = elem['id'];
//           var fp_waypointype = elem.getElementsByTagName("ATCWaypointType")[0].childNodes[0].nodeValue;
//           var fp_position_str= elem.getElementsByTagName("WorldPosition")[0].childNodes[0].nodeValue;
//           var position_items = fp_position_str.split(',');
//           var lat_part = position_items[0].split(/[^\d\w\.]+/);
//           var lat_hemisphere = lat_part[0][0];
//           var lat_degree = lat_part[0].slice(1) 
//           var lng_part = position_items[1].split(/[^\d\w\.]+/);
//           var lng_hemisphere = lng_part[0][0];
//           var lng_degree = lng_part[0].slice(1) 
//           var lat = ConvertDMSToDD(lat_degree, lat_part[1], lat_part[2], lat_hemisphere); //degrees, minutes, seconds, hemisphere
//           var lng = ConvertDMSToDD(lng_degree, lng_part[1], lng_part[2], lng_hemisphere);
//           var latLng = new google.maps.LatLng(lat, lng);
//           addPoiToFlightPlan(lat, lng, fp_name, fp_waypointype)
...
//         console.error(error);
//         flash_error_importing_flightplan();
//       }
//   }
...
// function save_fp_as_recorded_flight() {

//   flight_details = {}

//   //as this is coming from a flight plan the aircraft details are unkown
//   flight_details['aircraft_name'] = ''
//   flight_details['aircraft_rego'] = ''

//   flight_positions = [];

//   flightPath_data = getFlightPathData();

//   for (let i = 0; i < flightPath_data.length; i++) {

//     lat = flightPath_data[i]['latLng'][0];
//     lng = flightPath_data[i]['latLng'][1];
//     position = {'user_lat':lat, 'user_lng':lng, 'ground_speed':5, 'altitude':0, 'altitude_agl':0};  //dummy ground speed as recorded flight wont save if GS == 0
//     flight_positions.push(position);
//   }
...
//     contentType: "application/json",
//     success: function(response) {
//       console.log("successfully recorded flight from flight plan");
//       $("#save_fp_as_recorded_flight_label").text("Flight plan saved as a recorded flight");
//       $("#save_fp_as_recorded_flight").prop("disabled", true);
//       flightplan_saves_as_recorded_flight = true;
      
//     },
//     error: function(error) {
//          console.log("PROBLEM saving recorded flight from flight plan");
//         console.log(error);
//     }
//   })
// }
...
//     body_html = "<p>Please add at least one waypoint to create a flight plan.  On the map, click on a 'Point of Interest' or airport and select 'add to Flight Plan'</p>";
//     body_html += "<br>";
//     body_html += "<p>Alternatively, you can import a previously created flight plan (*.pln)</p>";
...
//       if (flightplan_saves_as_recorded_flight) {
//         hide_div = 'hidden'
//       }
...
//       body_html += '<input type="checkbox" class="form-check-input" style="cursor: pointer;" id="export-share_fp">';
//       body_html += "<label class='mr-2 mb-0' style='cursor: default;'>Save & Share Flight Plan</label>";
//       body_html += "</div>";
//       body_html += "</form>";
//       body_html += "<hr>";
//       body_html += "<form>";
//       body_html += "<div class='form-check mt-1'"+ hide_div + ">";
//       body_html += '<input type="checkbox" class="form-check-input" id="save_fp_as_recorded_flight" style="cursor: pointer;" title="In case you forgot to record a previous flight">';
//       body_html += "<label id='save_fp_as_recorded_flight_label' class='mr-2 mb-0' style='cursor: default;' >Save Plan as a Recorded Flight</label>";
//       body_html += "</div>";
//       body_html += "</form>";
//       body_html += 
//         `<script>
...
// function flash_error_importing_flightplan() {

//   $("#flash_error_importing_flightplan").show();
//     setTimeout(function() { $("#flash_error_importing_flightplanh").hide(); }, 10000);
// }
